...
function removeLastOccurrences(str, value) {
  ...
  const endIndex = str.lastIndexOf(value) + value.length - 1;
  const firstPart = str.slice(0, startIndex);
  const lastPart = str.slice(endIndex + 1, str.length);
...
function countVowels(str) {
  ...
  let counter = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str.toLowerCase()[i])) counter += 1;
  }
  return counter;
}
...
function isPalindrome(str) {
  ...
  return newStr === Array.from(newStr).reverse().join('');
}
...
function reverseWords(str) {
  ...
  const newSentence = [];
  words.forEach((word) => {
    newSentence.push(Array.from(word).reverse().join(''));
  });
...
function getCardId(value) {
  ...
  const curDenomination = value.replace(curSuit, '');
  const tens = suits.findIndex((suit) => suit === curSuit) * 13;
  const units =
    denominations.findIndex(
      (denomination) => denomination === curDenomination
    ) + 1;
  return tens + units - 1;
}
